...
class Node extends EventEmitter {
  ...
  onmessage (packet, sender) {
    ...
    switch (opcode) {
      ...
      case packets.turn: {
        this.onTurn(data, sender)
        break
      }
      case packets.ack_turn: {
        this.onTurnAcknowledged(data, sender)
        break
      }
    }
  }
  ...
  onRegisterAcknowledged (data, sender) {
    ...
    const peer = this.createPeer(data[0], sender)
    // TEMPORARY: disable this so the system will mock a restricted network so nodes will have to use TURN server.
    this.endHandshakeTimeout(peer)
  ...
  onNewPeer (data, sender) {
    ...
    if (this.registerPeer(peer)) {
      this.startHandshakeTimeout(peer, sender)
      this.send(this.createPacket(packets.register, this.id), peer.endpoint)
    } else {
      // Peer already exists in peerlist
    }
  ...
  startHandshakeTimeout (peer, relay) {
    this.handshakeTimeouts[peer.id] = { id: peer.id, start: new Date(), relay }
    setTimeout(() => {
      if (!this.handshakeTimeouts[peer.id].hasOwnProperty('end')) {
        this.relays[peer.id] = { peer, alive: false } // TODO: make it so that instead of using this.relays we can use this.peerlist for turn servers too.
        this.send(this.createPacket(packets.turn, this.id, peer.id), relay)
      } else {
        delete this.handshakeTimeouts[peer.id]
      }
    }, 4000)
  }

  endHandshakeTimeout (peer) {
    if(this.handshakeTimeouts.hasOwnProperty(peer.id)) {
      const handshakeTimeout = this.handshakeTimeouts[peer.id]
      handshakeTimeout.end = new Date()
    }
  }

  onTurn (data, sender) {
    const pa = this.peerlist[data[0]]
    const pb = this.peerlist[data[1]]
    const turnId = crypto.createHmac('sha512', this.id).update(pa.id + pb.id).digest('hex').substring(0, 40)
    this.turn[turnId] = { from: pa, to: pb }
    this.send(this.createPacket(packets.ack_turn, pb.id, this.id, turnId), sender)
  }

  onTurnAcknowledged (data, sender) {
    const receiverId = data[0]
    const brokerId = data[1]
    if(this.relays.hasOwnProperty(receiverId) && this.peerlist.hasOwnProperty(brokerId)) {
      const peer = this.relays[receiverId].peer
      peer.broker = this.peerlist[brokerId]
      peer.turnId = data[2]
      this.relays[receiverId].alive = true
      if (this.peerlist.hasOwnProperty(receiverId) && this.peerlist[receiverId].alive === true) {
